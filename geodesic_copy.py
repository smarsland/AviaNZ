...
def geod_sphere(beta1, beta2, k=5, scale=False, rotation=True, center=True):
    ...
    """
    lam = 0.0
    returnpath = 0 #we don't need this
    n, T = beta1.shape
    ...
    innerprod_calc = cf.innerprod_q2(q1, q2n)
    #fix to not create problem with arccos
    if innerprod_calc > 1:
        innerprod_calc = 1
    elif innerprod_calc < -1:
        innerprod_calc = -1
    dist = arccos(innerprod_calc)
