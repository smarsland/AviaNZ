...
def summarize(viz_folder):
    ...
    header = (
        ...
        f"{'AviaNZ Acc':>10}  {'AviaNZ F1':>10}  {'AviaNZ Jaccard':>14}  "
        f"{'DOC Acc':>7}  {'DOC F1':>7}  {'DOC Jaccard':>11}"
    )
    print(header)
    print("-" * len(header))
    ...
    for exp in experiments:
        ...
        for key, path in [("avianz", avianz_file), ("doc", doc_file)]:
            if path:
                ...
                vals[key] = {"acc": None, "f1": None, "jaccard": None}
        ...
        print(
            f"{exp:<{col_exp}}  "
            f"{fmt(a['acc']):>10}  {fmt(a['f1']):>10}  {fmt(a['jaccard']):>14}  "
            f"{fmt(d['acc']):>7}  {fmt(d['f1']):>7}  {fmt(d['jaccard']):>11}"
        )
