...
Problem: If experiments halt mid-run, some experiment folders are created
         but don't have a result.json file. Lock files (.lock) may also
         remain from crashed experiments.

Solution: Delete experiment folders without result.json AND remove stale
          lock files from both the main experiments directory AND the
...
def main():
    ...
    # Count complete, incomplete, and stale locks
    incomplete = []
    complete = []
    stale_locks = []
    ...
    print("Scanning for incomplete experiments and stale locks...")
    ...
    for exp_name in sorted(exp_names):
        ...
        if not has_result:
            ...
            if results_exp_dir and (results_exp_dir / ".lock").exists():
                lock_file = results_exp_dir / ".lock"
                lock_age = time.time() - lock_file.stat().st_mtime
                lock_age_hours = lock_age / 3600
                
                print(f"🔒 STALE LOCK: {exp_name} (lock age: {lock_age_hours:.1f} hours)")
                stale_locks.append(exp_name)
                lock_file.unlink()
            ...
            # Complete experiment - check for stale lock anyway
            if results_exp_dir and (results_exp_dir / ".lock").exists():
                lock_file = results_exp_dir / ".lock"
                print(f"⚠️  Complete but has lock: {exp_name}")
                lock_file.unlink()
                print(f"   → Removed lock file from completed experiment")
    ...
    print(f"  Stale locks removed    : {len(stale_locks)}")
    print("=" * 70)
    print()
