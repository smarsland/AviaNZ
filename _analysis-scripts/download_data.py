...
## USAGE: python download_data.py AUDIODIR
## will download audio files to AUDIODIR
...
import sys
AUDIODIR = sys.argv[1]
print("Downloading files to", AUDIODIR)
...
if __name__ == "__main__":
    ...
    # complete evaluation filelist - identical to metadata/eval/eval.csv in DCASE repository
    log.line("Eval")
    fset1 = os.path.join("filelists", "eval.csv")
    result_dir = os.path.join(AUDIODIR, "eval")
    download(fset1, result_dir, n_jobs=N_JOBS, chunk_size=CHUNK_SIZE)

    # Five files containing cat examples for WF training
    log.line("Train-cats")
    fset2 = os.path.join("filelists", "train-wf.csv")
    result_dir = os.path.join(AUDIODIR, "train-wf")
    download(fset2, result_dir, n_jobs=N_JOBS, chunk_size=CHUNK_SIZE)

    # Five files containing speech & other examples, no cat sounds, for classifier training
    log.line("Train-neg")
    fset3 = os.path.join("filelists", "train-neg.csv")
    result_dir = os.path.join(AUDIODIR, "train-neg")
    download(fset3, result_dir, n_jobs=N_JOBS, chunk_size=CHUNK_SIZE)

    # Seven files containing cats & other examples, for evaluating WF
    log.line("Test")
    fset4 = os.path.join("filelists", "test.csv")
    result_dir = os.path.join(AUDIODIR, "test")
    download(fset4, result_dir, n_jobs=N_JOBS, chunk_size=CHUNK_SIZE)
