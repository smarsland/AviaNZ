...
header "Step 1 / 8 — Build matched dataset"
...
# ── Step 2: Build full-DOC dataset ────────────────────────────────────────
header "Step 2 / 8 — Build full-DOC dataset"
FULL_DOC_MARKER="${BASE}/full_doc/doc_large/labels.json"
if [[ "$OVERWRITE" -eq 1 || ! -f "$FULL_DOC_MARKER" ]]; then
    run bash build_full_doc_dataset.sh ${OVERWRITE:+--overwrite}
else
    echo "  [skip] full-DOC dataset — already exists"
fi

# ── Step 3: Build combined dataset ────────────────────────────────────────
header "Step 3 / 8 — Build combined dataset"
...
# ── Step 4: Matched experiments ───────────────────────────────────────────
header "Step 4 / 8 — Matched experiments (baseline + bgsub + BirdNET + Kaytoo)"
...
# ── Step 5: Full-DOC experiment ───────────────────────────────────────────
header "Step 5 / 8 — Full-DOC experiment (RegNet +BgSub)"
...
# ── Step 6: Combined pre-train ────────────────────────────────────────────
header "Step 6 / 8 — Combined pre-train (RegNet +BgSub)"
...
# ── Step 7: Combined finetune ─────────────────────────────────────────────
header "Step 7 / 8 — Combined finetune (RegNet +BgSub → matched)"
...
# ── Step 8: Analyse + generate figure ─────────────────────────────────────
header "Step 8 / 8 — Analyse results + generate summary figure"
