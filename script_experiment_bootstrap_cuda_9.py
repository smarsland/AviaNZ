...
# save labels
results_directory = "/home/listanvirg/Documents/Individual_identification/Test_results/Bootstrap_tests_9"
    ...
    for k in range(5):
        ...
        for i in range(n2):
            ...
            for j in range(n1):
                ...
                df1_matrix[i, j] = np.abs(new_reference_freq_curve[-1, 1] -
                                          train_freq_curves[j, len_freq_list1[j] - 1, 1])
        ...
        file_txt.writelines(np.concatenate((l0, l1, l2, l3, l5)))
        file_txt.close()
        ...
        np.savetxt(result_folder + "/IntialFreq"+ str(k)+".txt", df0_matrix, fmt='%s')
        np.savetxt(result_folder + "/FinalFreq."+ str(k)+"txt", df1_matrix, fmt='%s')
        np.savetxt(result_folder + "/MaxFreq"+ str(k)+".txt", dfmax_matrix, fmt='%s')

        del ssd_matrix, geod_matrix, pca_matrix, dtw_matrix, df_matrix, df0_matrix, df1_matrix, dfmax_matrix
