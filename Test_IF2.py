...
# test_fold="exponential_downchirp"
...
#inst_freq=omega*np.ones((np.shape(t)))
...
# s1_inverted=s1_inverted[192:-512]
# s1_inverted_or=s1_inverted/(np.ptp(s1_inverted)/np.ptp(s1))
# s1_inverted_data=s1_inverted/(np.ptp(s1_inverted)/np.ptp(s1_data))
# # plt.plot(s1[1024:-1024],'b')
# # plt.plot(s1_inverted)
# # plt.show()
# #t_inverted=np.linspace(0,T, len(s1_inverted))
# #s1_inverted=np.interp(t,t_inverted[128:], s1_inverted[128:])
# # plt.plot(s1_inverted)
# # plt.show()
#
# #plt.plot(s1_inverted)
...
# #reconstr signal
# # plt.plot(s1_recostructed)
# # plt.show()
# # fig,ax=plt.subplots(2,1)
# # ax[0].plot(s1)
# # ax[1].plot(s1_recostructed)
# # plt.show(fig)
# #L2_rec=norm(s1-s1_recostructed, ord=2)
# #KLD_rec=Kul_Lieb_Div(s1_recostructed,s1)
# #inverted signal
# # L2_inv=norm(s1[512:-512]-s1_inverted[192:-512], ord=2)
# # KLD_inv=Kul_Lieb_Div(s1[512:-512],s1_inverted[192:-512])
# L2_inv=norm(s1[512:-512]-s1_inverted, ord=2)
# KLD_inv=Kul_Lieb_Div(s1[512:-512],s1_inverted)
# L2_inv_data=norm(s1_data[512:-512]-s1_inverted_data, ord=2)
# KLD_inv_data=Kul_Lieb_Div(s1_data[512:-512],s1_inverted_data)
# #Renyi entropy
...
# fieldnames=['std','L2 if', 'KLD if', 'L2 reconstructed','KLD reconstructed','L2 inverted','KLD inverted','Renyi Entropy']
# with open(test_dir+'\\'+test_fold+'\\' + '\\baseline_values.csv', 'w', newline='') as csvfile:
#     writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
#     writer.writeheader()
#     writer.writerow({'std': std_s1, 'L2 if': L2_IF, 'KLD if': KLD_IF,
#                      'L2 reconstructed': L2_rec,'KLD reconstructed':KLD_rec,'L2 inverted':L2_inv,'KLD inverted':KLD_inv,
#                      'Renyi Entropy': RE_0 })
...
for test_fold in os.listdir(test_dir):
    ...
    s1=sp.data
    fs = sp.sampleRate
    TFR_0 = sp.spectrogram(window_width, incr, window)
    TFR_0 = TFR_0.T
    del sp
    mean=0
    var=1
    ...
    for dir in os.listdir(save_directory):
        ...
        for file in os.listdir(save_directory+'\\'+dir):
            ...
            # inverted signal
            L2_inv = norm(sig1[512:-512] - s1_inverted_data, ord=2)
            L2_inv_G[k,i-1]=L2_inv
            KLD_inv = Kul_Lieb_Div( sig1[512:-512],s1_inverted_data)
            KLD_inv_G[k,i-1]=KLD_inv

            #inverted signal VS original
            L2_inv_or = norm(s1[512:-512] - s1_inverted_or, ord=2)
            L2_inv_or_G[k, i - 1] = L2_inv_or
            KLD_inv_or = Kul_Lieb_Div(s1[512:-512], s1_inverted_or)
