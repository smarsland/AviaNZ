...
\todo{Redo some figures. Consider images of a single dock in relevant places}
...
You can also view a restricted amount of the spectrogram by reducing the visible frequency band. To do this non-destructively, use the `Change spectrogram parameters' dialog in the Appearance menu, and change the Lowest and Highest Frequency options. There are other options in that dialog too, which are concerned with the way that the spectrogram is made. Feel free to try them. %For more information about them, see Section~\ref{sec:spectrogram}.
\todo{Point form information is really easy to follow. So turn the following paragraphs into point form where possible?}
...
\todo{Include play, playSegment, playBandlimited buttons? or the Controls area? Going back to see Figure 1 seems interrupting}
...
\item [Mark on spectrogram] There are some features that will help you spot bird calls or recognise them. We currently offer three options here: the fundamental frequency, spectral derivatives, and points of high energy. %See Section~\ref{sec:spectrogram} for more details.
...
\begin{figure}[h!]
\centering
\includegraphics[width=.75\textwidth]{Figs/confusionMatrix.png}
\end{figure}
