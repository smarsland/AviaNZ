...
When the program loads, you are presented with two options \added{as shown} in the following screen:
...
When you select manual segmentation, \deleted{you should see a dialog box asking you to select a file to view. Use this in the normal way to select a sound file from a directory. Once you have selected one,} you should see a screen like the one below \added{(Figure~\ref{main})}. This is the main program for manually labelling birdcalls, or reviewing the processing of automatic processes.
The program automatically loads the first 5 minutes of a file. 

\begin{figure}[h!]
\centering
\includegraphics[width=.8\textwidth]{Figs/avianzinterface.pdf}
\caption{\added{The interface for manual birdsong annotation and analysis.}}
\label{main}
\end{figure}
...
\item If you want to move to a new directory, either use the `..' option at the top of the list of files to navigate around your computer's file system, \deleted{or use the  `Open sound file' in the File menu.} 
...
\item The part of the file you can see can be changed by:
	...
	\item changing the `Visible \added{window width (seconds)}' in the Controls dock (labelled 17) by clicking on the up and down arrows, or typing in a new number.
	\end{itemize}

\subsection{Moving Through Long Files}

\item If files are longer than 5 minutes, use the double arrows labelled (5) to move to the next or previous sections. Note that there is a 10 second overlap between the files. \added{You can also change the page size and the amount of page overlap by choosing the relevant options in the `Interface Settings' in the Interface menu.} The times on the axis below the spectrogram show locations in the full file.
...
\item To segment a birdcall, click the left mouse button once on either the Amplitude or Spectrogram plot. Then, as you move the mouse (within the same plot) a blue box will follow the mouse. Click again to \deleted{stop the segmentation} \added{finish the segment}. The box will turn green, and a drop-down menu will then appear asking you to label the segment. 
...
\item These colours match the rectangles underneath the overview (labelled 3). \added{By default, f}or each 10 second segment of the file, these boxes are white if there are no segments, blue if there are don't know segments, yellow if there are question marked segments, or red if all the segments in that section are labelled with definite species. To move to a particular box, click on it and the amplitude and spectrogram plots will show that section. 

\item Segments can be moved by dragging them, or opened out by dragging the left and right ends, just like the Overview highlight. \added{For the boxes, click on the top right corner of the box and then drag to resize.}

\item Segments can be deleted by selecting them (so that they turn green) and then clicking on the `Delete Current Segment' in the Controls (16). You can also just press the \deleted{delete}\added{backspace} key on the keyboard. 
...
\item \added{You also have the flexibility to set the colors for different types of the segments as you wish. To do so navigate to `Interface settings' in the Interface menu}
\end{itemize}

\subsection{Playing a Segment}

\begin{itemize}
\item When a segment is selected (so it is green \added{by default}) you can use the play button on the left of 14 in the picture to play just that small segment. You can also use the play button on the right. The difference is that the one on the right will play only the frequencies highlighted if you have dragged a limited box on the spectrogram, so that you can isolate particular frequencies of a call. \added{This is particularly helpful when there is high level of background noise overlapped temporally with the sound of interest but not overlapped in the frequency domain.} 
\end{itemize}
...
\item [\added{Choose c}olour map] This enables the user to select a colour map they prefer to the standard grey one. There is also an option to swap black and white (inverting the colour map). 
...
\item [Interface settings] \added{This option enables the user to customise the annotation color scheme, cell size of the annotation overview, and auto save frequency. The user can also choose a page size. There is a tick box under `Human classify`, this option is used when reviewing the automated segmentation results  (Section \ref{sec:action} and Sections \ref{sec:auto}--\ref{sec:outputs}).  There are situations that the automated detector outputs false positives (false alarms), then the user can delete them during the review. Here the user can set  the program to automatically save those segments in a temporary folder for us, then we can use this information to improve the detectors. 
\\
...
\\
There is an option to set the operator and reviewer, this facility can also be found under File menu.
} 
\end{description}
...
\item [Denoise] Runs some programs that try to get rid of the noise in the sound file so that the birdcalls are easier to see. \deleted{You need to choose parameters for the algorithms.
...
\item [Check Segments] These two options provide two different ways to view the segments and their labels for easier checking. The option to Show all pages decides on whether to view segments in an entire file, or just the current 5 minutes of it. 
	\begin{description}
	\item [Check segments [All segments]] This option shows the segments one-by-one in time order \added{(Figure~\ref{check1}).} 
	\begin{figure}
	\centering
	\includegraphics[width=.6\textwidth]{Figs/Check1}
	\caption{\added{Check segments [All segments].}}
	\label{check1}
	\end{figure}
	...
	\item [Check segments [Choose species]] This option asks you to choose a bird from those found in the current file. For each segment that is wrongly labelled, click on its picture (the colour map will change). These segments will be deleted \added{(Figure~\ref{check2}).} 
	\begin{figure}
	\centering
	\includegraphics[width=.4\textwidth]{Figs/Check2}
	\caption{\added{Check segments [Choose species].}}
	\label{check2}
	\end{figure}
	\end{description}

\item [Export segments to Excel] \added{This enables the user to save a summary of the manual annotation or automatic detections of the currently opened file into an excel. The excel is automatically creates in the same location where the sound file resides. This outputs the detections in three different forms: start and end of each segment, presence/absence of the target species within the file, and the presence/absence of the target species in each time interval that the user specify in automatic segmentation (or by default 1 second resolution).}
...
\label{sec:auto}
\added{When you select `Find a Species' from the starting window, you will see a screen like the one below (Figure~\ref{batch}). You can navigate to the parent folder with recordings to process, this means that the folder may or may not have sub folders to process. There is an option to choose the target species (e.g. Kiwi), then the automatic detector will be customised accordingly. If someone wants to detect any sound that stands out from the background, the option is to select `all' from the drop-down list. \\
When the user hits the `Process Folder' button the detector will start go through each file in the given folder and the cursor will turn into busy mode. It will take a considerable amount of time depending on the size of the dataset. The output resolution will be covered in next section.}

\begin{figure}[h!]
\centering
\includegraphics[width=.5\textwidth]{Figs/proc_folder}
\caption{\added{The interface for batch processing.}}
\label{batch}
\end{figure}

\section{Outputs}
\label{sec:outputs}
\added{The program is dedicated to provide a detailed and easy to review output. An excel file (with the name `DetectionSummary' followed by species selected) will be generated with three sheets of outputs in the parent directory. If you already have such a file there, the new results will be appended to the end of each sheet. The first sheet provides a detailed output -- the start and end of each detection in each recording. The second sheet indicates the presence/absence of the target species in each recording. The third sheet presents the presence/absence of the target species with a preferred resolution specified in the detection window (Output Resolution). This gives the user the flexibility to adopt their own output. \\
In addition to the excel output, AviaNZ generates an annotation file of the automated detections. Then the user can open and review them in the main interface.}
