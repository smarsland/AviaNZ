...
def update_confusion_matrix(filewise_output, segments_filewise_test):
    """
    ...
    for i in range(file_number):
        ...
        if correct_label==assigned_label:
            ...
            if correct_label=='Noise':
                missed_files.append([filewise_output[i][0], correct_label, assigned_label])
                ...
                false_positives.append([filewise_output[i][0], correct_label, assigned_label])
                if correct_label=='LT':
                ...
                if correct_label=='LT':
                    if assigned_label=='LT?':
                        ...
                        misclassified_files.append([filewise_output[i][0], correct_label, assigned_label])
...
test_fold= "BAT SEARCH TESTS\Test_01" #Test folder where to save all the stats
...
#    check: if there is noting in the directory: skip 
    ...
    for i in range(file_number):
        file = segments_filewise_test[i][0]
        control='False'
        click_label, test_featuress, count_end = ClickSearch(bat_dir+'\\'+dirname, file, test_featuress, count_start, Train=False)
    ...
    #recover model
    modelpath= "D:\Desktop\Documents\Work\Data\Bat\BAT\CNN experiment\TEST2\BAT SEARCH TESTS\Test_Spec_01\model_6.h5"
    ...
    confusion_matrix, comparison_annotations, missed_files, false_positives, misclassified_files= update_confusion_matrix(filewise_output, segments_filewise_test)
    confusion_matrix_tot+=confusion_matrix #updating confusion matrix tot
...
TD_rate= (TD/(file_number-np.sum(confusion_matrix_tot[:][5])))*100
print('True Detected rate', TD_rate)
