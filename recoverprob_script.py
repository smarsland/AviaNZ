...
#                print('check file_prob',file_prob)
               if target_label[k] == 0:
                   LT_probabilities.append([spec_id[k][0], segments_filewise_test[i][1], predictions[k][:].tolist()])
               elif target_label[k] == 1:
                   ST_probabilities.append([spec_id[k][0], segments_filewise_test[i][1],  predictions[k][:].tolist()])
               elif target_label[k] == 2:
                   Noise_probabilities.append([spec_id[k][0], segments_filewise_test[i][1],  predictions[k][:].tolist()])
        if click_detected_flag==False:
            Noise_probabilities.append([0, segments_filewise_test[i][1], np.zeros((1,3)).tolist()])
...
for i in range(file_number):
    file = segments_filewise_test[i][0]
    control='False'
    click_label, test_featuress, count_end = ClickSearch(test_dir, file, fs, test_featuress, count_start, Train=True)
...
LT_prob, ST_prob, Noise_prob =label_probability(predictions, spec_id, test_label, segments_filewise_test, filewise_output, file_number )
#save probability vectors
import json
f= open('D:\\Desktop\\Documents\\Work\\Data\\Bat\\BAT\\CNN experiment\\TEST2\\BAT SEARCH TESTS\\Test_79\\LT_spec_prob.data', 'w')
#for line in LT_prob:
 #   f.write(str(line))
  #  f.write('\n')
json.dump(LT_prob,f)
...
f= open('D:\\Desktop\\Documents\\Work\\Data\\Bat\\BAT\\CNN experiment\\TEST2\\BAT SEARCH TESTS\\Test_79\\ST_spec_prob.data', 'w')
#for line in ST_prob:
#    f.write(str(line))
#    f.write('\n')
json.dump(ST_prob,f)
...
f= open('D:\\Desktop\\Documents\\Work\\Data\\Bat\\BAT\\CNN experiment\\TEST2\\BAT SEARCH TESTS\\Test_79\\Noise_spec_prob.data', 'w')
