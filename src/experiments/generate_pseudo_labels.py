...
from src.core.models import AST, CNNModel
...
class PseudoLabeler:
    ...
    def run(self):
        ...
        print(f"     python train.py {self.data_folder} <new_output_folder> \\")
        print(f"       --model-type ast --pretrained {self.model_path} \\")
...
Examples:
  ...
  python src/experiments/generate_pseudo_labels.py doc_out_mult/ast_model_best.pt \\
  ...
  python src/experiments/generate_pseudo_labels.py model.pt config.json data/ data/labels_pseudo.json --soft-labels
  
  # Use higher threshold to be more conservative
  python src/experiments/generate_pseudo_labels.py model.pt config.json data/ data/labels_pseudo.json --threshold 0.7
  
  # Keep only top-3 predictions per sample
  python src/experiments/generate_pseudo_labels.py model.pt config.json data/ data/labels_pseudo.json --top-k 3

Workflow:
  1. Train initial model:
     python train.py data/ output1/ --model-type ast --epochs 25
  
  2. Generate pseudo-labels (this script):
     python src/experiments/generate_pseudo_labels.py output1/ast_model_best.pt output1/ast_model_config.json \\
  ...
  4. Train new model on pseudo-labels, initialized from first model:
     python train.py data/ output2/ --model-type ast \\
