...
class Spectrogram:
    ...
    def downsample_freq_bins(self, nfilters):
        """Downsample frequency bins for linear-scale spectrograms.
        
        Groups adjacent frequency bins by averaging to reduce from current 
        number of bins to nfilters bins. Used for standard spectrograms when
        a specific number of frequency bins is desired.
        
        Args:
            nfilters: Target number of frequency bins
        """
        current_bins = self.sg.shape[1]
        
        if current_bins == nfilters:
            return  # Already the right size
        
        if current_bins < nfilters:
            print(f"Warning: Cannot upsample from {current_bins} to {nfilters} bins. Keeping original.")
            return
        
        # Create pooling matrix that averages adjacent bins
        # Each output bin is the average of a contiguous range of input bins
        pooling_matrix = np.zeros((current_bins, nfilters))
        
        bins_per_filter = current_bins / nfilters
        
        for m in range(nfilters):
            # Determine which input bins contribute to this output bin
            start_bin = int(m * bins_per_filter)
            end_bin = int((m + 1) * bins_per_filter)
            
            # Handle edge case for last bin
            if m == nfilters - 1:
                end_bin = current_bins
            
            # Assign weights (simple averaging)
            n_contributing = end_bin - start_bin
            if n_contributing > 0:
                pooling_matrix[start_bin:end_bin, m] = 1.0 / n_contributing
        
        # Apply pooling
        try:
            self.sg = np.dot(self.sg, pooling_matrix)
        except:
            # Multi-channel case (e.g., multi-tapered)
            placeholder = np.zeros(shape=(self.sg.shape[0], nfilters, self.sg.shape[2]))
            for i in range(self.sg.shape[2]):
                placeholder[:,:,i] = np.dot(self.sg[:,:,i], pooling_matrix)
            self.sg = placeholder
    ...
    def spectrogram(self, window_width=None, incr=None, window='Hann', sgType='Standard', 
        ...
        if sgScale == 'Mel Frequency':
            ...
            # For linear scale, downsample frequency bins if nfilters is specified
            # and different from current number of bins
            if nfilters is not None and nfilters != self.sg.shape[1]:
                self.downsample_freq_bins(nfilters)
