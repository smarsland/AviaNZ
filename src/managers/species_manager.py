...
class SpeciesManager(QObject):
    ...
    def load_species_lists(self, config_dir):
        ...
        """
        from ..core.SupportClasses import ConfigLoader
        
        config_loader = ConfigLoader()
        
        species_data = {
            'short_list': config_loader.shortbl(self.config['BirdListShort'], config_dir),
            'long_list': config_loader.longbl(self.config['BirdListLong'], config_dir),
            'known_calls': config_loader.knownCalls(self.config['KnownCallsList'], config_dir),
            'bat_list': config_loader.batl(self.config['BatList'], config_dir)
        }
        
        # Validate critical lists
        if species_data['short_list'] is None:
            raise ValueError("Failed to load short bird list - check configuration")
    ...
    def scan_segments_for_species(self, segments_data):
        ...
        for seg_data in segments_data:
            ...
            if hasattr(segment, 'getKeysWithCalltypes'):
                for species, calltype in segment.getKeysWithCalltypes():
                    # Track known calls
                    if species not in known_calls:
                        known_calls[species] = []
                    if calltype not in known_calls[species] and calltype is not None:
                        known_calls[species].append(calltype)
                    
                    # Track new species for short list
    ...
    def update_short_list_from_segments(self, current_short_list, new_species):
        ...
        for species in new_species:
            if species not in updated_list:
                # Remove last item to maintain list size
                if len(updated_list) > 0:
                    del updated_list[-1]
                updated_list.append(species)
    ...
    def merge_known_calls(self, existing_calls, new_calls):
        ...
        for species, calltypes in new_calls.items():
            if species not in merged_calls:
                merged_calls[species] = []
            
            for calltype in calltypes:
                if calltype not in merged_calls[species]:
                    merged_calls[species].append(calltype)
    ...
    def save_species_lists(self, species_data, config_dir):
        ...
        # Sort long list before saving
        if 'long_list' in species_data and species_data['long_list']:
            species_data['long_list'] = sorted(species_data['long_list'], key=str.lower)
        
        # Save each list
        if 'long_list' in species_data:
            config_loader.blwrite(species_data['long_list'], self.config['BirdListLong'], config_dir)
        if 'short_list' in species_data:
            config_loader.blwrite(species_data['short_list'], self.config['BirdListShort'], config_dir)
        if 'known_calls' in species_data:
            config_loader.knownCallsWrite(species_data['known_calls'], self.config['KnownCallsList'], config_dir)
    ...
    def get_species_lists_for_dialog(self, species_data):
        ...
        return {
            ...
            'bat_list': species_data.get('bat_list', [])
        }
