...
class ManualInterface(QMainWindow):
    ...
    def createFrame(self):
        ...
        self.p_overview = DemousedViewBox()
        self.w_overview1.addItem(self.p_overview,row=0,col=0)
        self.p_overview2 = ChildInfoViewBox(enableMouse=False, enableMenu=False)
        ...
        self.p_ampl = DragViewBox(self, enableMouse=False,enableMenu=False,enableDrag=False, thisIsAmpl=True)
        ...
        self.p_spec = DragViewBox(self, enableMouse=False,enableMenu=False,enableDrag=self.config['specMouseAction']==3, thisIsAmpl=False)
        ...
        self.overviewImageRegion = LinearRegionItemO(pen=pg.mkPen(120,80,200, width=2),
        ...
        self.floorSlider = CustomSlider(Qt.Orientation.Horizontal)
        ...
        self.specControls = BrightContrVol(self.config['brightness'], self.config['contrast'], self.config['invertColourMap'], horizontal=False)
        ...
        self.listFiles = LightedFileList(self.ColourNone, self.ColourPossibleDark, self.ColourNamed)
        ...
        with pg.ProgressDialog("Loading..", 0, 6) as dlg:
            ...
            if name is not None:
                ...
                if DOCRecording:
                    ...
                    self.timeaxis = TimeAxisHour(orientation='bottom',linkView=self.p_ampl)
                else:
                    self.startTime = 0
                    self.timeaxis = TimeAxisMin(orientation='bottom',linkView=self.p_ampl)
            ...
            if not self.CLI:
                if not self.batmode:
                    ...
                    self.media_obj = ControllableAudio(self.sp,useBar=True)
    ...
    def openNextFile(self):
        ...
        if i+1<len(self.listFiles):
            ...
            msg = MessagePopup("d", "Last file", "You've finished processing the folder")
            msg.exec()
    ...
    def drawOverview(self):
        ...
        for i in range(numSegments):
            r = ClickableRectItem(i*self.widthOverviewSegment, 0, self.widthOverviewSegment, 1)
    ...
    def addSegment(self,startpoint,endpoint,y1=0,y2=0,species=[],saveSeg=True,index=-1,remaking=False,coordsAbsolute=False):
        ...
        p_ampl_r = LinearRegionItem2(self, brush=self.prevBoxCol, movable=segsMovable, bounds=[0, self.datalengthSec])
        ...
        if self.batmode:
            ...
            p_spec_r = None
            p_spec_r = LinearRegionItem2(self, pen=pg.mkPen(self.prevBoxCol, width=6), movable=segsMovable, bounds=[0, np.shape(self.sg)[0]])
        ...
        # rectangle boxes:
            ...
            p_spec_r = ShadedRectROI(startpointS, endpointS - startpointS, movable=segsMovable, maxBounds=scenerect, parent=self)
            if self.config['transparentBoxes']:
            ...
            # If this is the first click:
                ...
                if evt.button() == self.MouseDrawingButton:
                    ...
                    if self.config['specMouseAction']>1:
                        ...
                        self.drawingBox_spec = ShadedRectROI(startpointS, endpointS - startpointS, invertible=True)
    ...
    def annotJumper(self, maxcert):
        ...
        if targetix is None:
            ...
            msg = MessagePopup("w", "No more annotations", "No further annotation to jump to found")
            msg.exec()
            return
        ...
        if target.start_time>self.startRead + self.datalengthSec:
            ...
            if pagenum > self.nFileSections:
                ...
                QApplication.restoreOverrideCursor()
                msg = MessagePopup("w", "No more annotations", "No further annotation to jump to found in this sound file")
                msg.exec()
                return
...
# Generate the various dialogs that match the menu items
    ...
    def showDiagnosticDialog(self):
        ...
        if not hasattr(self, 'diagnosticDialog'):
            self.diagnosticDialog = Diagnostic(self.FilterDicts)
    ...
    def showSpectrogramDialog(self):
        ...
        if not hasattr(self,'spectrogramDialog'):
            self.spectrogramDialog = SpectrogramDialog(self.config['window_width'],self.config['incr'],self.sp.minFreq,self.sp.maxFreq, self.sp.minFreqShow,self.sp.maxFreqShow, self.config['windowType'], self.config['sgType'], self.config['sgNormMode'], self.config['sgScale'], self.config['nfilters'],self.batmode)
    ...
    def spectrogram(self):
        ...
        if (newMinFreqShow >= newMaxFreqShow):
            msg = MessagePopup("w", "Error", "Incorrect frequency range")
            msg.exec()
            return
        with pg.BusyCursor():
    ...
    def showShapesDialog(self):
        ...
        self.shapesDialog = ShapesDialog()
    ...
    def showDenoiseDialog(self):
        ...
        """
        self.denoiseDialog = Denoise(DOC=self.DOC,minFreq=self.sp.minFreq,maxFreq=self.sp.maxFreq)
    ...
    def denoise_save(self):
        ...
        msg = MessagePopup("d", "Saved", "Destination: " + '\n' + filename)
        msg.exec()
        return
    ...
    def saveSelectedSound(self,changespeed):
        ...
        if self.box1id is None or self.box1id<0:
            ...
            msg = MessagePopup("w", "No segment", "No sound selected to save")
            msg.exec()
            return
        else:
            if type(self.listRectanglesa2[self.box1id]) == self.ROItype:
...
# Code for playing sounds
    ...
    def setOperatorReviewerDialog(self):
        ...
        if hasattr(self, 'operator') and hasattr(self, 'reviewer') :
            self.operatorReviewerDialog = OperatorReviewer(operator=self.operator,reviewer=self.reviewer)
        else:
            self.operatorReviewerDialog = OperatorReviewer(operator='', reviewer='')
    ...
    def manageFilters(self):
        filterManagerSimple = FilterManager(self.filtersDir)
