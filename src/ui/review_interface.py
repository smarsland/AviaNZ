...
class ReviewInterface(QMainWindow):
    ...
    def review(self, quick=False):
        ...
        if len(self.allSegments) == 0:
            msg = MessagePopup("w", "No Segments", "No segments found to review!")
            msg.exec()
            return
    ...
    def finishReview(self, cnt, total, filesuccess):
        ...
        if filesuccess == 1:
            ...
            msg = MessagePopup("w", "Review stopped", msgtext)
            msg.setStandardButtons(QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
            reply = msg.exec()
            if reply == QMessageBox.StandardButton.Yes:
                QApplication.exit(1)
    ...
    def exportExcel(self):
        ...
        if excsuccess!=1:
            ...
            print(msgtext)
            msg = MessagePopup("w", "Failed to export Excel file", msgtext)
        else:
            msgtext = "Excel output is stored in " + os.path.join(self.dirName, "DetectionSummary_*.xlsx")
            msg = MessagePopup("d", "Excel output produced", msgtext)
        msg.exec()
    ...
    def review_single(self, filename, chunksize):
        ...
        self.humanClassifyDialog2 = HumanClassify2(self.sps, sgs, self.segments, self.indices2show,
