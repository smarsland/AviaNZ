...
count=0
CNN=np.zeros((len(test_id),1))
CD=np.zeros((len(test_id),1))
CD_p=np.zeros((len(test_id),1))
Lab=np.zeros((len(test_id),1))
for test in test_id:
    CNN[count]=0
    CD[count]=0
    CD_p[count]=0
    ...
    for dir in dir_path_list:
        fp_file=dir+'\\Test_New_'+str(test)+'\\false_positives.csv'
        fn_file=dir+'\\Test_New_'+str(test)+'\\missed_files.csv'
        misc_file=dir+'\\Test_New_'+str(test)+'\\misclassified_files.csv'
        file_list=[fp_file, fn_file, misc_file]
        for file in file_list:
            with open(file, 'r') as csvfile:
                csvreader=csv.DictReader(csvfile)
                for row in csvreader:
                    if dict(row)["Possible Error"]=='CNN':
                        CNN[count]+=1
                    elif dict(row)["Possible Error"]=='CD':
                        CD[count]+=1
                    elif dict(row)["Possible Error"]=='CD?':
                        CD_p[count]+=1
                    elif dict(row)["Possible Error"]=='Label':
                        Lab[count]+=1
    count+=1
print(np.shape(CNN))
print(type(CNN))
...
for k in range(0,4):
    ...
    for i in range(0,3):
    #ax.set_ylim(40,100)
        index_conf_0=slice(3*k+i,73,12)
        print(i, index_conf_0)
        ax[0,i].set_title('File Label'+str(i))
        ax[0,i].plot(test_id[index_conf_0],Recall[index_conf_0], 'r', label='Rec')
        ax[0,i].plot(test_id[index_conf_0], Precision_pre[index_conf_0], 'b', label='Pre_pre')
        ax[0,i].plot(test_id[index_conf_0], Precision_post[index_conf_0], 'g', label='Pre_post')
        ax[0,i].legend(loc='lower right')

        x = np.arange(len(test_id[index_conf_0]))  # the label locations
