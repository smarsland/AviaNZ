...
class CompareCallsDialog(QDialog):
    ...
    def setData(self):
        ...
        maxsg = max(np.max(self.sp1.sg), np.max(self.sp2.sg))
        minsg = min(np.min(self.sp1.sg), np.min(self.sp2.sg))
        ...
        # 0.3 - temp br, 0.8 - temp contr
        self.colourStart = 0.3 * 0.8 * (maxsg-minsg) + minsg
        self.colourEnd = self.colourStart + (1-0.8)*(maxsg-minsg)
        # TODO no color sliders yet
        # self.setColourLevels()
        self.plot1.setLevels([self.colourStart, self.colourEnd])
        self.plot2.setLevels([self.colourStart, self.colourEnd])
        # No scroll area yet
