...
for i in range(start_i,6):
    ...
    for j in range(start_j,4):
        ...
        for k in range(file_number_train):
            ...
            sp.readBmp2(filepath, rotate=False)
            #read annotaion file
        ...
        train_images = x_train.reshape(x_train.shape[0],first_dim, 64, 1) #changed image dimensions
        validation_images = x_validation.reshape(x_validation.shape[0],first_dim, 64, 1)
        input_shape = (first_dim, 64, 1)
        ...
        accuracies=np.zeros((5,1)) #initializing accuracies array
                    ...
                    for file in files:
                        ...
                        if file.endswith('.bmp'):
                            ...
                            if click_label=='Click':
                                ...
                                x_test = sg_test
                                test_images = x_test.reshape(x_test.shape[0],first_dim, 64, 1)
                                input_shape = (first_dim, 64, 1)
