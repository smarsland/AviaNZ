...
def ClickSearch(imspec, label):
    ...
    #thr_spec=(np.mean(mean_spec_all))*np.ones((np.shape(mean_spec)))*0.65
...
test_num=22
Test_fold="Test_"+str(test_num)
data_fold=root_path+"\\Data"

if Test_fold not in os.listdir(root_path):
    os.mkdir(root_path+"\\"+Test_fold)
...
for root, dirs, files in os.walk(data_fold):
    ...
    for file in files:
        ...
        #CLickSearch
        detected_clicks, count=ClickSearch4(sp.sg,label)
        ...
        f = open(root_path+"\\"+Test_fold+"\\"+file +'.data', 'w')
...
cd_metrics_file=root_path+"\\"+Test_fold+'\\click_detector_stats.txt'
