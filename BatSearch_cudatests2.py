...
def update_confusion_matrix(saving_dir, dir, file, assigned_label, correct_label, confusion_matrix):
    """
    ...
    if correct_label==assigned_label:
        ...
        if correct_label=='Noise':
            update_csv(saving_dir+'/false_positives.csv', dir, file, correct_label, assigned_label)
            ...
            update_csv(saving_dir+'/missed_files.csv', dir, file, correct_label, assigned_label)
            if correct_label=='Long-tailed bat':
            ...
            if correct_label=='Long-tailed bat':
                if assigned_label=='Long-tailed bat?':
                    ...
                    update_csv(saving_dir+'/misclassified_files.csv', dir, file, correct_label, assigned_label)
...
def update_csv(csvfilename, dir, file, correct_label, assigned_label):
    """
    ...
    fieldnames=['Directory', 'Filename', 'True Label', 'Assigned Label']
    with open(csvfilename, 'a', newline='') as csvfile:
        writer=csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writerow({'Directory':dir, 'Filename': file, 'True Label':correct_label, 'Assigned Label': assigned_label})
...
for i in range(1,2):
    ...
    for j in range(4):
                ...
                for file in files:
                    ...
                    if file.endswith('.bmp'):
                        ...
                        if click_label=='Click':
                            ...
                            #for each classes
                            label=File_label(predictions, thr1, thr2, bat_dir_local_storage, file)
                        ...
                        confusion_matrix= update_confusion_matrix(test_sub_dir, bat_dir[-8:], file, assigned_label, GT_label , confusion_matrix)
        ...
        with open(test_general_results_dir+ '/' + test_fold+'/confusion_matrix.csv', 'w', newline='') as csvfile:
                ...
                writer.writeheader()
                for i in range(7):
                    writer.writerow({'  ':column_fieldnames[i], 'LT':confusion_matrix_tot[i][0], 'LT?':confusion_matrix_tot[i][1], 'ST':confusion_matrix_tot[i][2], 'ST?':confusion_matrix_tot[i][3], 'BT':confusion_matrix_tot[i][4], 'Noise':confusion_matrix_tot[i][5]})
