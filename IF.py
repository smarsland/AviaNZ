    ...
    def pathopt(self,Number_peaks,Indeces_peaks,Frequency_peaks,Wp_inst,logw1, logw2,freq):
        ...
        if isinstance(logw1, types.LambdaType):
            for tn in range(tn1+1,tn2+1):
                print(np.shape(Frequency_peaks[0:Number_peaks[0,tn]+1,tn]),np.shape(np.ones((1,Number_peaks[0,tn-1]))))
                ...
                U[0:Number_peaks[0,tn]+1,tn]=np.amax(aid_matrix,1)#max along  rows
                del aid_matrix
        else:
            for tn in range(tn1+1,tn2+1):
