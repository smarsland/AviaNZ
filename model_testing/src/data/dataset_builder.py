...
def load_data(source_type, input_folder, output_folder, window_seconds=None, hop_seconds=None,
    ...
    """
    Main entry point for data processing. Loads audio data and converts to spectrograms or WAV segments.
    
    Args:
        source_type: Type of data source ('avianz', 'doc')
        input_folder: Path to input audio folder
        output_folder: Path to output folder
        window_seconds: Spectrogram window width in seconds
        hop_seconds: Spectrogram hop length in seconds
        freq_bins: Number of frequency bins for spectrogram
        fs: Sample rate for WAV extraction
        overwrite: Whether to overwrite existing output folder
        output_format: Output format ('spectrogram' or 'wav')
        ignore_multilabel: Whether to ignore multi-label segments
        with_audio: Whether to save audio files alongside spectrograms
        audioset_fbank: Whether to use AudioSet-compatible fbank features
    ...
    if source_type == 'avianz':
        ...
        if output_format != 'spectrogram':
            raise ValueError("Noise source only supports spectrogram output format")
        processor = NoiseDataProcessor(spec_processor, segment_extractor, output_format, with_audio)
        file_count = processor.process(
            input_folder=input_folder,
            output_folder=output_folder,
        ...
        raise ValueError(f"Unknown source type: {source_type}. Supported types: 'avianz', 'doc', 'noise'")
...
Output Formats:
  ...
  wav         - Extract as WAV files

Examples:
  # Process AviaNZ annotations to spectrograms
  python dataset_builder.py avianz "Sound Files/GSK" "Sound Files/GSK_spec"
  
  # Limit number of segments
  python dataset_builder.py avianz "Sound Files/GSK" "Sound Files/GSK_spec" --max-segments 1000
  
  # Process specific species using eBird codes
  python dataset_builder.py avianz "Sound Files/GSK" "Sound Files/GSK_spec" --species "morepo2,tui1"
  
  # Extract WAV files instead of spectrograms
  python dataset_builder.py avianz "Sound Files/GSK" "Sound Files/GSK_wav" --format wav
  
  # Process DOC birddata
  python dataset_builder.py doc "Sound Files/NZ bird sounds" "Sound Files/DOC_spec" --species "tui1,bellbi1"
  
  # Include audio with spectrograms
  python dataset_builder.py doc "Sound Files/NZ bird sounds" "Sound Files/DOC_spec" --with-audio
  
  # Extract DOC WAV files with sample limit
  ...
  python dataset_builder.py avianz "Sound Files/GSK" "Sound Files/GSK_spec" --window 0.025 --hop 0.010 --freq-bins 128
    ...
    parser.add_argument('source_type', type=str, choices=['avianz', 'doc', 'noise', 'esc', 'inference'],
                       help="Type of data source to process (avianz: AviaNZ annotations; doc: DOC bird sounds; noise: environmental audio; esc: ESC-50 dataset; inference: unlabeled audio)")
